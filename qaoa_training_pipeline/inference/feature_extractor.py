...
if TYPE_CHECKING:  # torch is an export/dev dependency only — not imported at runtime.
    import torch
...
class AIFeatureExtractor:
    ...
    Example:
        extractor = FeatureExtractor(in_features=["num_nodes", "mean_degree"])
        features = extractor.extract(cost_op)
        x_vec = extractor.pack_features(features)
    """
    ...
    def extract(self, cost_op: SparsePauliOp) -> dict[str, Any]:
        """Extract raw (unnormalized) features from a cost operator.

        Rescales the operator by `rescaling_factor`, converts to a graph, and computes
        the same scalar/graph features used during training.
        """
        import torch  # torch-typed path; lazily imported so the numpy path stays torch-free.
        ...
        features: dict[str, Any] = {
            ...
            "edges": torch.as_tensor(
                edge_list if edge_list else [[0, 0]], dtype=torch.long
            ).unsqueeze(0),
            "edge_weights": torch.as_tensor(
                edge_weights if edge_weights else [0.0], dtype=torch.float32
            ).unsqueeze(0),
            "node_count": torch.tensor([num_nodes], dtype=torch.long),
            "t": torch.zeros(1, dtype=torch.long),
        }
        return features

    def pack_features(
        self, features: dict[str, torch.Tensor], device: str | torch.device = "cpu"
    ) -> torch.Tensor:
        """
        Pack the requested `in_features` into a single tensor in
        sorted-name order, applying normalization
        for entries listed in `norm_stats`.
        """
        import torch  # lazily imported; see extract().

        values: list[float] = []
        for name in sorted(self.in_features):
            if name not in features:
                raise KeyError(f"FeatureExtractor: feature {name!r} not produced by extract()")
    ...
    def extract_and_pack(
        self, cost_op: SparsePauliOp, device: str | torch.device = "cpu"
    ) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        """
        Extract and pack features in one call.

        Args:
            cost_op: QAOA cost operator
            device: Device to place tensors on

        Returns:
            Tuple of (packed_tensor, feature_dict)
        """
        import torch  # lazily imported; see extract().

        features = self.extract(cost_op)
        ...
        x_vec = self.pack_features(features, device=device)

        return x_vec, features

    # ------------------------------------------------------------------
    # Torch-free (numpy) path — used by the ONNX runtime.
    # Mirrors extract()/pack_features()/extract_and_pack() exactly but emits
    # numpy arrays and imports no torch. Kept separate so the torch-typed
    # methods above (and their tests) are untouched.
    # ------------------------------------------------------------------

    def extract_np(self, cost_op: SparsePauliOp) -> dict[str, Any]:
        """Numpy analogue of ``extract`` — same raw features, numpy arrays.

        Returns the same scalar features as ``extract`` plus:
            edges         (1, M, 2) int64
    ...
    def pack_features_np(self, features: dict[str, Any]) -> np.ndarray:
        """Numpy analogue of ``pack_features`` — (1, F) float32 in sorted order."""
        values: list[float] = []
        for name in sorted(self.in_features):
            if name not in features:
    ...
    def extract_and_pack_np(self, cost_op: SparsePauliOp) -> tuple[np.ndarray, dict[str, Any]]:
        """Numpy analogue of ``extract_and_pack``. Returns ``(x_vec, features)``."""
